...
def service_from_pod_name(pod):
    pod = str(pod)
    for service in KNOWN_HIPSTER_SERVICES:
        if service in pod:
            return service
...
def same_service(pod_a, pod_b):
    return service_from_pod_name(pod_a) == service_from_pod_name(pod_b)
...
def pattern_ranker(normal_pattern_dict, normal_event_graphs, abnormal_time, ns, log_template_miner,topk=10, min_score=0.67):
    ...
    # Add metric alarms as standalone RCA candidates. In small local
    # experiments an abnormal metric may not attach to a deep trace pattern,
    # but it is still a valid service-level signal.
    for item in alarm_list:
        for alarm in item.get("alarm", []):
            metric_type = alarm["metric_type"]
            alarm_event = log_parsing(log=metric_type, pod="alarm", log_template_miner=log_template_miner)
            result_list.append({
                "events": "%s_%s" % (alarm_event, alarm_event),
                "score": 1.0,
                "deepth": 100,
                "pod": item["pod"],
    ...
    for i in range(len(fault_inject_list)):
        ...
        for hour in fault_inject_data:
            for fault in fault_inject_data[hour]:
                ...
                inject_service = service_from_pod_name(fault["inject_pod"])
                root_cause = root_cause_list[inject_service][fault["inject_type"]].split(
                    "_")
                for i in range(len(result_list)):
                    if same_service(result_list[i].get("pod", ""), fault["inject_pod"]):
                        top_list.append(topk)
                        logger.info("%s Inject Ground Truth: %s, %s score %s", fault["inject_time"],
                                    fault["inject_pod"], fault["inject_type"], topk)
                        break
