...
func main() {
	...
	mustMapEnv(&svc.imageSvcAddr, "IMAGE_SERVICE_ADDR")
	...
	r.HandleFunc("/product/{id}/comment", svc.addCommentHandler).Methods(http.MethodPost)
	r.HandleFunc("/promo-image", svc.promoImageHandler).Methods(http.MethodGet)
	r.HandleFunc("/orders", svc.viewOrderHistoryHandler).Methods(http.MethodGet)
