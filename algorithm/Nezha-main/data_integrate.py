...
def process_pool_workers(task_count):
    """Pick a Windows-safe process pool size."""
    if task_count <= 0:
        return 1
    cpu_count = os.cpu_count() or 1
    # Windows caps ProcessPoolExecutor at 61 workers.
    return max(1, min(task_count, cpu_count, 61))
...
def data_integrate(trace_file, trace_id_file, log_file, alarm_list, ns,log_template_miner):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=process_pool_workers(len(trace_id_reader[0]))) as executor1:
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=process_pool_workers(len(log_sequences))) as executor2:
