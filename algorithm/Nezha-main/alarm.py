...
def get_netwrok_metric(trace_file, pod_name):
    ...
    try:
        ...
        return float(pod_reader.iloc[0].iloc[0]), 0
...
def determine_alarm(pod, metric_type, metric_value, std_num, ns):
    ...
    if metric_type == "CpuUsageRate(%)":
        if metric_value > 80:
            return True
    elif metric_type == 'MemoryUsageRate(%)':
        # The local minikube experiment uses a larger assumed memory limit
        # than the injected process actually consumes. Treat a clear rise
        # above the normal productcatalog baseline as a memory alarm.
        if "productcatalogservice" in pod and metric_value > 30:
            return True
    else:
        ...
        if "metric" in path:
            ...
            for index in range(len(metrics['Time'])):
                ...
                if re.search(time, metrics['Time'][index]):
                    ...
                    if 'NetworkP90(ms)' in metrics.columns and not pd.isna(metrics['NetworkP90(ms)'][index]):
                        network_p90 = float(metrics['NetworkP90(ms)'][index])
                    else:
